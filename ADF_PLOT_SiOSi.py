...
def main():
    ...
    file_paths = [
        input(f"Enter the path for File {i} ({(i)*5}% Na₂O): ")
        for i in range(8)
    ]
    ...
    colors = ['blue', 'black', 'orange', 'red', 'purple', 'pink', 'brown', 'green']
    labels = [f'{(i)*5}% Na₂O' for i in range(8)]
    ...
    plt.ylabel(r'ADF ($\hat{\theta}_{SiOSi}$)', fontsize=14)  # y-axis label with a hat
    ...
    image_path = os.path.join(script_dir, 'plot_ADF_SiOSi_smooth.png')
