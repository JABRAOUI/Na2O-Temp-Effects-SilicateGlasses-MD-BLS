...
def main():
    ...
    file_paths = [
        input(f"Enter the path for File {i} ({(i+1)*5}% Na₂O): ")
        for i in range(8)  # Only 7 files for 0% to 35% Na₂O
    ]
    ...
    colors = ['blue', 'black', 'orange', 'red', 'purple', 'pink', 'brown', 'green']  # Colors for 5% to 35%
    labels = [f'{(i)*5}% Na₂O 300K' for i in range(8)]  # Labels for 0% to 35%
    ...
    plt.ylabel('RDF ($r_{Si-Si}$)', fontsize=14)  # Update ylabel to indicate offset
    ...
    image_path = os.path.join(script_dir, 'plot_RDF_SiSi_curves_300K_0_to_35.png')
