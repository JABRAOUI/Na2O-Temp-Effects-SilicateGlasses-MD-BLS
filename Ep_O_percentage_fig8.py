...
# Composition data: {Na2O%: number_of_oxygen_atoms}
composition_data = {
    0: 6400, 5: 6240, 10: 6080, 15: 5920,
    20: 5760, 25: 5600, 30: 5440, 35: 5280
}
...
all_values = []
for i in i_values:
    data = read_data_from_file(f"O_PD_{i}_300K.txt")
...
for idx, i in enumerate(i_values):
    data = read_data_from_file(f"O_PD_{i}_300K.txt")
    values = np.array([val for val, count in data.items() for _ in range(int(count))])
    total_oxygen = composition_data[i]
    
    # Plot histogram showing percentage of oxygen atoms
    counts, bins, _ = axes[idx].hist(values, bins=50, alpha=0.6, color='blue',
                                   weights=np.ones_like(values)/total_oxygen*100,
    ...
    axes[idx].set_xlabel(r'E$_p$ [eV]', fontsize=10)
    axes[idx].set_ylabel('% of O atoms', fontsize=10)
...
plt.savefig("Ep_O_percentage.png", dpi=300, bbox_inches='tight')
plt.show()
